...
pub async fn populate(r: &mut PostureReport) {
    ...
    if let Some(out) = run_ps(r#"(Get-ItemProperty 'HKCU:\Control Panel\Desktop' -ErrorAction SilentlyContinue).ScreenSaverIsSecure"#) {
        r.screen_lock.enabled = out.trim() == "1";
    }
    if let Some(out) = run_ps(r#"(Get-ItemProperty 'HKCU:\Control Panel\Desktop' -ErrorAction SilentlyContinue).ScreenSaveTimeOut"#) {
    ...
    if let Some(out) = run_ps(r#"(Get-NetFirewallProfile | Where-Object { -not $_.Enabled } | Measure-Object).Count"#) {
    ...
    if let Some(out) = run_ps(r#"(Get-MpComputerStatus -ErrorAction SilentlyContinue).RealTimeProtectionEnabled"#) {
    ...
    // Remote access
    if let Some(out) = run_ps(r#"(Get-Service -Name TermService -ErrorAction SilentlyContinue).Status"#) {
