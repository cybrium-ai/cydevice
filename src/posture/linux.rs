...
pub async fn populate(r: &mut PostureReport) {
    ...
    if let Ok(out) = Command::new("blkid").args(["-t", "TYPE=crypto_LUKS"]).output() {
    ...
    if !r.disk_encryption.enabled {
        if let Ok(s) = std::fs::read_to_string("/etc/crypttab") {
            if s.lines().any(|l| !l.trim().is_empty() && !l.starts_with('#')) {
                r.disk_encryption.enabled = true;
    ...
    if let Ok(out) = Command::new("gsettings").args(["get", "org.gnome.desktop.screensaver", "lock-enabled"]).output() {
        let s = String::from_utf8_lossy(&out.stdout).trim().to_string();
        r.screen_lock.enabled = s == "true";
    }
    if let Ok(out) = Command::new("gsettings").args(["get", "org.gnome.desktop.session", "idle-delay"]).output() {
    ...
    if !r.firewall.enabled {
        if let Ok(out) = Command::new("systemctl").args(["is-active", "nftables"]).output() {
            if out.status.success() {
                r.firewall.enabled = true;
    ...
    for svc in ["clamav-daemon", "clamd", "falcon-sensor", "sentinelone-agent"] {
    ...
    if let Ok(out) = Command::new("apt").args(["list", "--upgradable"]).output() {
        ...
        if let Some(code) = out.status.code() {
            if code == 100 {
                let s = String::from_utf8_lossy(&out.stdout);
                r.os_updates.pending_updates = s.lines().filter(|l| !l.trim().is_empty()).count() as i32;
            }
        }
    }

    // Remote access
    if let Ok(out) = Command::new("systemctl").args(["is-active", "ssh"]).output() {
        if out.status.success() {
            r.remote_access.ssh_enabled = true;
        }
    }
    if !r.remote_access.ssh_enabled {
        if let Ok(out) = Command::new("systemctl").args(["is-active", "sshd"]).output() {
            if out.status.success() {
                r.remote_access.ssh_enabled = true;
            }
        }
